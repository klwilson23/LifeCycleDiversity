...
av_abund_wild <- c(1.73e6, 0.88e6, 4.5e5)
av_abund_total <- c(1.73e6, 0.88e6, 1.71e6)
meta_abund_wild <- c(1.73e6, 0.88e6, 4.5e5)
meta_abund_total <- c(1.73e6, 0.88e6, 1.71e6)
av_cv_wild <- c(0.977, 1.005, 0.537)
av_cv_total <- c(0.977, 1.005, 0.573)
meta_cv_wild <- c(0.483, 0.677, 0.424)
meta_cv_total <- c(0.483, 0.677, 0.548)

#name the periods
names(meta_abund_wild) <- names(meta_abund_total) <- names(meta_cv_wild) <- names(meta_cv_total) <- names(av_cv_wild) <- names(av_cv_total) <- Period <- c("1913-1923","1933-1947","2010-2017")
...
Ntotal <- closure <- matrix(NA, Nboots, Nperiods, 
                                 ...
                                 "Period" = c("1913-1923","1933-1947","2010-2017")))
...
for(p in 1:Nperiods)
{
  for(bI in 1:Nboots) #for each bootstrap iteration
  {
    ...
    meta_Ntot[bI,p] <- max(0,rnorm(1,mean=meta_abund_total[p],sd=meta_cv_total[p]*meta_abund_total[p]))
    # (1b) - repeat this but for wild abundance/CV only
    meta_Nwild[bI,p] <- max(0,rnorm(1,mean=meta_abund_wild[p],sd=meta_cv_wild[p]*meta_abund_wild[p]))
    ...
    #(1) generate random normal draw for metapopulation abundance given mean N and the meta CV
    av_Ntot[bI,p] <- max(0,rnorm(1,mean=av_abund_total[p],sd=av_cv_total[p]*av_abund_total[p]))
    # (1b) - repeat this but for wild abundance/CV only
    av_Nwild[bI,p] <- max(0,rnorm(1,mean=av_abund_wild[p],sd=av_cv_wild[p]*av_abund_wild[p]))
    # calculate whether the wild-only fishery closed
...
metaClosureWild <- as.data.frame(meta_closure_wild) %>%
  ...
  filter(period == "1913-1923") %>% #mannually change for each time-period
...
#create data.frames for Wild & Total components for plotting (1 & 2)
...
ggTotal <- rbind(avTot, metaTot) %>%
  ...
  filter(period == "2010-2017") #filter for recent period b/c only these data required for plot
...
ggplot(ggWild, aes(x=period, y=abundance, fill=component)) +
  ...
  geom_violin(data = ggTotal, color="red", alpha=.2, aes(fill=component)) +
  ggsave(file = paste0("plots/Fishery_closure_combined.pdf")) 
